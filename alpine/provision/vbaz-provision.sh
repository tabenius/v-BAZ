...
for _m in /etc/vbaz/vbaz-offline.sh /etc/vbaz/vbaz-wifi.sh /etc/vbaz/vbaz-storage.sh /etc/vbaz/vbaz-runtimes.sh /etc/vbaz/vbaz-rebekah.sh /etc/vbaz/vbaz-guest.sh /etc/vbaz/vbaz-thinpool.sh /etc/vbaz/vbaz-secureboot.sh; do
    # shellcheck disable=SC1090
    [ -f "$_m" ] && . "$_m"
done
...
main() {
    ...
    command -v setup_guest    >/dev/null 2>&1 && setup_guest
